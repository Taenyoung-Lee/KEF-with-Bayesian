...
```{r}
# ELBO with Q = lambda*K + eps*I, A = H + Q
...
# closed-form posterior mean alpha (EB)
...
ebf_fit <- function(X, sigma_init=NULL, lambda_init=1.0, eps=1e-6,
                    ...
                    lambda_bounds=c(1e-8,1e6), verbose=FALSE){
  X <- as.matrix(X); Tn <- nrow(X); d <- ncol(X)
  if (standardize){ x_center <- colMeans(X); x_scale <- apply(X,2,sd);
  ...
  list(X=X, sigma=sigma, lambda=lambda, eps=eps, elbo=Lcur, alpha=sol$alpha,
       H=mats$H, K=mats$K, b=mats$b, x_center=if(exists("x_center")) x_center else NULL,
       x_scale=if(exists("x_scale")) x_scale else NULL, standardize=standardize)
}
# predict (EB)
predict_f_eb <- function(fit, Xnew){
  ...
  Tn <- nrow(fit$X); out <- matrix(0, nrow(Xnew), Tn)
  for (i in 1:nrow(Xnew)){
    diff <- sweep(fit$X, 2, Xnew[i,], FUN="-"); r2 <- rowSums(diff^2)
...
```{r}
# GBI ELBO with Q=lambda*K+eps*I, R=tau*H + Q
...
fit_gbi <- function(X, sigma_init=NULL, lambda_init=1.0, tau_init=1.0, eps=1e-6,
                    ...
                    standardize=TRUE, sigma_clip_factor=c(0.2,5.0),
                    lambda_bounds=c(1e-8,1e6), tau_bounds=c(1e-3,1e3), verbose=FALSE){
  X <- as.matrix(X); Tn <- nrow(X); d <- ncol(X)
  if (standardize){ x_center <- colMeans(X); x_scale <- apply(X,2,sd);
  ...
  mats <- build_mats(X, sigma)
  Lcur <- GBI_elbo(mats$K, mats$H, mats$B <- mats$b, lambda, tau, eps)
  for (it in 1:max_iter){
    improved <- FALSE
  ...
  list(X=X, sigma=sigma, lambda=lambda, tau=tau, eps=eps, elbo=Lcur, m=post$m,
       H=mats$H, K=mats$K, b=mats$b, x_center=x_center, x_scale=x_scale, standardize=standardize)
}
# predict (GBI)
...
# EB
fit_eb <- ebf_fit(X1_mix, eps=1e-6, verbose=TRUE)
xs <- seq(min(X1_mix)-3, max(X1_mix)+3, length.out=400)
d_eb <- predict_den_eb(fit_eb, xs)
# GBI
fit_gb <- fit_gbi(X1_mix, eps=1e-6, verbose=TRUE)
d_gb <- predict_den_gbi(fit_gb, xs)
par(mfrow=c(1,2))
hist(X1_mix, breaks=30, freq=FALSE, main="EB (τ=1)", xlab="x", col="grey90", border="white")
lines(xs, d_eb/max(d_eb)*max(hist(X1_mix, breaks=30, plot=FALSE)$density), lwd=2)
hist(X1_mix, breaks=30, freq=FALSE, main="GBI (τ optimized)", xlab="x", col="grey90", border="white")
lines(xs, d_gb/max(d_gb)*max(hist(X1_mix, breaks=30, plot=FALSE)$density), lwd=2)
par(mfrow=c(1,1))
```
...
# EB
fit_eb <- ebf_fit(X1_gamma, eps=1e-6, verbose=TRUE)
xs <- seq(min(X1_gamma)-1, max(X1_gamma)+1, length.out=400)
d_eb <- predict_den_eb(fit_eb, xs)
# GBI
fit_gb <- fit_gbi(X1_gamma, eps=1e-6, verbose=TRUE)
d_gb <- predict_den_gbi(fit_gb, xs)
par(mfrow=c(1,2))
hist(X1_gamma, breaks=30, freq=FALSE, main="EB (τ=1)", xlab="x", col="grey90", border="white")
lines(xs, d_eb/max(d_eb)*max(hist(X1_gamma, breaks=30, plot=FALSE)$density), lwd=2)
hist(X1_gamma, breaks=30, freq=FALSE, main="GBI (τ optimized)", xlab="x", col="grey90", border="white")
lines(xs, d_gb/max(d_gb)*max(hist(X1_gamma, breaks=30, plot=FALSE)$density), lwd=2)
par(mfrow=c(1,1))
```
...
```{r}
set.seed(111)
X1_tri <- matrix(c(rnorm(150, -2, 0.6), rnorm(100, 0, 1.0), rnorm(150, 3, 0.8)), ncol = 1)
# EB
fit_eb <- ebf_fit(X1_tri, eps=1e-6, verbose=TRUE)
xs <- seq(min(X1_tri)-3, max(X1_tri)+3, length.out=500)
d_eb <- predict_den_eb(fit_eb, xs)
# GBI
fit_gb <- fit_gbi(X1_tri, eps=1e-6, verbose=TRUE)
d_gb <- predict_den_gbi(fit_gb, xs)
par(mfrow=c(1,2))
hist(X1_tri, breaks=30, freq=FALSE, main="EB (τ=1)", xlab="x", col="grey90", border="white")
lines(xs, d_eb/max(d_eb)*max(hist(X1_tri, breaks=30, plot=FALSE)$density), lwd=2)
hist(X1_tri, breaks=30, freq=FALSE, main="GBI (τ optimized)", xlab="x", col="grey90", border="white")
lines(xs, d_gb/max(d_gb)*max(hist(X1_tri, breaks=30, plot=FALSE)$density), lwd=2)
par(mfrow=c(1,1))
```
...
# EB
fit_eb <- ebf_fit(X1_t, eps=1e-6, verbose=TRUE)
xs <- seq(min(X1_t)-3, max(X1_t)+3, length.out=500)
d_eb <- predict_den_eb(fit_eb, xs)
# GBI
fit_gb <- fit_gbi(X1_t, eps=1e-6, verbose=TRUE)
d_gb <- predict_den_gbi(fit_gb, xs)
par(mfrow=c(1,2))
hist(X1_t, breaks=30, freq=FALSE, main="EB (τ=1)", xlab="x", col="grey90", border="white")
lines(xs, d_eb/max(d_eb)*max(hist(X1_t, breaks=30, plot=FALSE)$density), lwd=2)
hist(X1_t, breaks=30, freq=FALSE, main="GBI (τ optimized)", xlab="x", col="grey90", border="white")
lines(xs, d_gb/max(d_gb)*max(hist(X1_t, breaks=30, plot=FALSE)$density), lwd=2)
par(mfrow=c(1,1))
```
...
# EB
fit_eb <- ebf_fit(X2_banana, eps=1e-6, verbose=TRUE)
par(mfrow=c(1,2))
plot_density_2d(X2_banana, fit_eb, predict_den_eb, "EB (τ=1)")
# GBI
fit_gb <- fit_gbi(X2_banana, eps=1e-6, verbose=TRUE)
plot_density_2d(X2_banana, fit_gb, predict_den_gbi, "GBI (τ optimized)")
par(mfrow=c(1,1))
```
