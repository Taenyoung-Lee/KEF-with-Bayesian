...
> 각 데이터마다 **방법2(EB)**와 **방법1(GBI)** 순서로 별도 코드 청크를 제공합니다.
...
## 1D Bimodal Mixture (sigma initialize problem?)
...
# EB with p0
fit_eb <- ebf_fit(X1_mix, eps=1e-6, score_p0 = score_p0_gaussian, sigma_init = 0.7)
xs <- seq(min(X1_mix)-2, max(X1_mix)+2, length.out=400)
d_eb <- dens1d_eb(fit_eb, xs)

# GBI with p0
fit_gb <- fit_gbi(X1_mix, eps=1e-6, score_p0 = score_p0_gaussian, sigma_init = 0.7)
d_gb <- dens1d_gbi(fit_gb, xs)
...
# EB with p0
fit_eb <- ebf_fit(X1_triple, eps=1e-6, score_p0 = score_p0_gaussian)
xs <- seq(min(X1_triple)-2, max(X1_triple)+2, length.out=400)
d_eb <- dens1d_eb(fit_eb, xs)

# GBI with p0
fit_gb <- fit_gbi(X1_triple, eps=1e-6, score_p0 = score_p0_gaussian)
d_gb <- dens1d_gbi(fit_gb, xs)

# True density
d_true <- (50/total_samples) * dnorm(xs, -4, 0.5) + 
          (150/total_samples) * dnorm(xs, 0, 0.8) + 
          (100/total_samples) * dnorm(xs, 3, 0.6)

par(mfrow=c(1,2))
hist(X1_triple, breaks=30, freq=FALSE, main="EB with p0=N(0,1)", xlab="x", col="grey90", border="white", ylim=range(0, d_eb, d_gb, d_true))
lines(xs, d_eb, lwd=2, col="blue")
lines(xs, d_true, lwd=2, lty=2, col="gray40") # True density line
legend("topright", legend=c("Estimate", "True"), col=c("blue", "gray40"), lty=c(1,2), lwd=2, bty="n")

hist(X1_triple, breaks=30, freq=FALSE, main="GBI with p0=N(0,1)", xlab="x", col="grey90", border="white", ylim=range(0, d_eb, d_gb, d_true))
lines(xs, d_gb, lwd=2, col="red")
lines(xs, d_true, lwd=2, lty=2, col="gray40") # True density line
legend("topright", legend=c("Estimate", "True"), col=c("red", "gray40"), lty=c(1,2), lwd=2, bty="n")
...
# EB with p0
fit_eb <- ebf_fit(X2_moons, eps=1e-6, score_p0 = score_p0_gaussian)
# GBI with p0
fit_gb <- fit_gbi(X2_moons, eps=1e-6, score_p0 = score_p0_gaussian)

par(mfrow=c(1,2))
plot_density_2d(X2_moons, fit_eb, predict_den_eb, "EB with p0=N(0,I)")
plot_density_2d(X2_moons, fit_gb, predict_den_gbi, "GBI with p0=N(0,I)")
par(mfrow=c(1,1))
```
...
# EB with p0
fit_eb <- ebf_fit(X2_plus, eps=1e-6, score_p0 = score_p0_gaussian)
# GBI with p0
fit_gb <- fit_gbi(X2_plus, eps=1e-6, score_p0 = score_p0_gaussian)

par(mfrow=c(1,2))
plot_density_2d(X2_plus, fit_eb, predict_den_eb, "EB with p0=N(0,I)")
plot_density_2d(X2_plus, fit_gb, predict_den_gbi, "GBI with p0=N(0,I)")
par(mfrow=c(1,1))
```

# 메모
- EB/VEM 섹션의 `eps`는 고정 jitter(안정화용)이며, 최적화하지 않습니다.
- GBI 섹션의 `tau`는 가능도 가중치로, likelihood에 영향력을 얼마나 곱해줄지(데이터가 제공하는 정보의 신뢰도도).
