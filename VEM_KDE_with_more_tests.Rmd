...
# 데이터 생성 (테스트 케이스 확장)
# ==============================
# NOTE: 기존 구조는 그대로 유지합니다. 아래 TEST_CASE만 바꿔서 다양한 분포를 실험하세요.
# 선택지: "gauss_mixture2" (기존), "gauss_mixture3", "t_heavytail", "skewed_lognormal", "beta_bounded", "crescent_moons"
TEST_CASE <- getOption("TEST_CASE", default = "gauss_mixture2")  # R 옵션으로도 제어 가능

generate_data <- function(case = "gauss_mixture2", n = 200) {
  set.seed(20250909)
  if (case == "gauss_mixture2") {
    # [기존] 2개 가우시안 혼합
    pi <- c(0.55, 0.45)
    ...
    X  <- matrix(NA, n, 2)
    X[z==1,] <- mvtnorm::rmvnorm(sum(z==1), mean = mu[[1]], sigma = S1)
    X[z==2,] <- mvtnorm::rmvnorm(sum(z==2), mean = mu[[2]], sigma = S2)
    return(X)
  } else if (case == "gauss_mixture3") {
    # 3개 가우시안 혼합 (불균형 + 이방성 공분산)
    pi <- c(0.6, 0.3, 0.1)
    mu <- list(c(-2, 0), c(2.5, 1.5), c(0, -2.5))
    S1 <- matrix(c(0.7, 0.4, 0.4, 0.9), 2, 2)
    S2 <- matrix(c(0.4, -0.25, -0.25, 0.6), 2, 2)
    S3 <- matrix(c(0.2, 0.0, 0.0, 0.8), 2, 2)
    z  <- sample(1:3, size = n, replace = TRUE, prob = pi)
    X  <- matrix(NA, n, 2)
    X[z==1,] <- mvtnorm::rmvnorm(sum(z==1), mean = mu[[1]], sigma = S1)
    X[z==2,] <- mvtnorm::rmvnorm(sum(z==2), mean = mu[[2]], sigma = S2)
    X[z==3,] <- mvtnorm::rmvnorm(sum(z==3), mean = mu[[3]], sigma = S3)
    return(X)
  } else if (case == "t_heavytail") {
    # 이변량 t(df=3) (heavy tail). 공분산 S를 가진 정규 g와 카이제곱 u로 구성해 스케일링.
    df <- 3
    mu <- c(0, 0)
    S  <- matrix(c(1.0, 0.5, 0.5, 1.5), 2, 2)
    Z  <- mvtnorm::rmvnorm(n, mean = c(0,0), sigma = S)
    U  <- rchisq(n, df = df)  # 각 표본마다 스케일
    X  <- sweep(Z, 1, sqrt(U/df), "/") + matrix(mu, n, 2, byrow = TRUE)
    return(X)
  } else if (case == "skewed_lognormal") {
    # 왜도 있는 로그정규형: 정규 생성 후 exp 변환 + 중심 이동
    mu_n  <- c(0.1, -0.2)
    S_n   <- matrix(c(0.4, 0.25, 0.25, 0.6), 2, 2)
    Z     <- mvtnorm::rmvnorm(n, mean = mu_n, sigma = S_n)
    X     <- exp(Z)  # 양의 영역, 강한 왜도
    X     <- scale(X, center = FALSE, scale = apply(X, 2, sd))  # 스케일 정규화
    X     <- X + matrix(c(-0.5, 0.5), n, 2, byrow = TRUE)       # 위치 이동
    return(X)
  } else if (case == "beta_bounded") {
    # 경계가 있는 분포: Beta(a,b) x Beta(a,b), [0,1]^2 상의 데이터
    a1 <- 2.0; b1 <- 5.0
    a2 <- 0.8; b2 <- 2.0
    x1 <- rbeta(n, a1, b1)
    x2 <- rbeta(n, a2, b2)
    X  <- cbind(x1, x2)
    # VEM 코드의 그리드와 스케일이 [-3,3] 주변에 맞춰져 있다면, 약간 선형변환
    X  <- (X - 0.5) * 6/1.0  # [0,1] -> [-3,3] 근방
    return(X)
  } else if (case == "crescent_moons") {
    # 초승달 모양(비선형): 두 개의 호(arc)에 노이즈 추가
    n1 <- round(0.5*n); n2 <- n - n1
    theta1 <- runif(n1, 0, pi)
    theta2 <- runif(n2, 0, pi)
    r1 <- 2 + rnorm(n1, sd = 0.1)
    r2 <- 2 + rnorm(n2, sd = 0.1)
    X1 <- cbind(r1*cos(theta1), r1*sin(theta1))
    X2 <- cbind(r2*cos(theta2) + 1.5, -r2*sin(theta2) + 0.5)
    X  <- rbind(X1, X2) + matrix(rnorm(2*n, sd = 0.05), n, 2)
    return(X)
  } else {
    stop(sprintf("Unknown TEST_CASE: %s", case))
  }
}

# 샘플 수
n <- 200
X_data <- generate_data(TEST_CASE, n = n)
...
다음 청크는 본문에서 정의된 `generate_data()`와 `run_vem_final()`을 그대로 사용하여
여섯 가지 TEST_CASE를 순차적으로 실행합니다. 각 케이스마다
- 데이터 산점도
- sigma, tau, ELBO 수렴 곡선

을 생성합니다. (밀도 윤곽선은 VEM 파라미터에서 로그정규화 상수가 명시적이지 않아, 본 섹션에서는 수렴/데이터 중심으로 간단히 요약합니다.)

```{r run-all-tests, message=FALSE, warning=FALSE}
library(ggplot2)
library(gridExtra)

all_cases <- c("gauss_mixture2",
               "gauss_mixture3",
               "t_heavytail",
               "skewed_lognormal",
               "beta_bounded",
...
for (case in all_cases) {
  cat("\n===== Running case:", case, "=====\n")
  X_data <- generate_data(case, n = 200)
  basis_pts <- X_data
  vem_results <- run_vem_final(X_data, basis_pts, n_iters = 600, lr_sigma = 1e-5)

  # 산점도
  df <- data.frame(x1 = X_data[,1], x2 = X_data[,2])
  p_data <- ggplot(df, aes(x1, x2)) + geom_point(alpha = 0.6, size = 1.2) +
    labs(title = paste0("[", case, "] 데이터 산점도")) + theme_bw()

  # 수렴 곡선 (history: iter, elbo, sigma, tau)
  history <- na.omit(vem_results$history)
  p_sigma <- ggplot(history, aes(iter, sigma)) + geom_line() +
    labs(title = "sigma 수렴", y = "sigma") + theme_bw()
  p_tau   <- ggplot(history, aes(iter, tau)) + geom_line() +
    labs(title = "tau 수렴", y = "tau") + theme_bw()
  p_elbo  <- ggplot(history, aes(iter, elbo)) + geom_line() +
    labs(title = "ELBO 수렴", y = "ELBO") + theme_bw()

  grid.arrange(p_data, p_sigma, p_tau, p_elbo, ncol = 2)

  # 보관
  results_list[[case]] <- list(history = history,
                               final_params = vem_results$final_params)
}

# 요약 테이블
summary_df <- do.call(rbind, lapply(names(results_list), function(k) {
  h <- results_list[[k]]$history
  fin <- tail(h, 1)
  data.frame(
    case = k,
    iter = fin$iter,
...
```{r save-summary-csv, message=FALSE, warning=FALSE}
# 결과 요약을 csv로 저장 (옵션)
out_csv <- "VEM_all_tests_summary.csv"
write.csv(summary_df, file = out_csv, row.names = FALSE)
cat("요약 저장:", normalizePath(out_csv), "\n")
```
