...
### 4.3. VEM 알고리즘 구현: 핵심 루프
 ...
 `compute_Kb` 함수와 전체 VEM 알고리즘 루프입니다. 
...
run_vem <- function(X_data, basis_pts, n_iters = 50, lr_sigma = 0.01) {
  ...
  for (iter in 1:n_iters) {
    ...
    # 1. 먼저 예측 오차의 기댓값 E[J(w)]를 계산합니다.
    expected_error <- 0.5 * (sum(diag(K_sigma %*% S)) + t(m) %*% K_sigma %*% m) - t(b_sigma) %*% m
    
    # 2. beta를 기댓값의 역수로 업데이트합니다. (오차가 0이 되는 경우 방지)
    beta <- 1 / max(expected_error, 1e-6) 
    # 3. sigma 업데이트 (경사 상승법)
